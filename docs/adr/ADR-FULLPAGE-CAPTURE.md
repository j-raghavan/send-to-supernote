...
- A **content script** (injected on demand via `scripting.executeScript`) measures the layout (`scrollHeight`, `clientHeight`, `devicePixelRatio`, scroll width), neutralizes `position: fixed/sticky` elements (so headers/banners don't repeat on every tile — restored afterwards), scrolls step-by-step, waits for paint + lazy-load at each step, and reports the scroll offsets. It **never persistently mutates** the page (scroll position and styles are restored — the I-4 "don't mutate the live page" principle).
- The **background** (event page / service worker) calls `captureVisibleTab` per step, **throttled** to the per-second capture quota, collecting the data-URL tiles.
- A **target-gated stitch core** (reusing the FF4 `__TARGET__` render split — offscreen document on Chrome, event-page DOM on Firefox) composites the tiles onto a canvas and **paginates to PDF via jsPDF** (Supernote-friendly), slicing the tall image into page-height chunks. A tall PNG is an optional secondary output.
...
### D4 — A revived `fullpage` capture mode; pipeline unchanged
Re-introduce `CaptureMode = 'reader' | 'fullpage'`, a Full Page option in the popup + a context-menu item, and `settings.defaultMode`. Everything downstream (md5, apply→PUT/POST→finish, jobs/retry/notifications) is unchanged — Full Page just produces a blob like Reader does.
