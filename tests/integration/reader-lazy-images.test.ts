...
describe('parseReader keeps capture-inlined images on lazy-loader pages', () => {
  it.each([
    // data-* BEFORE src (the order lazysizes markup is authored in) also
    // exercises the attribute-name (`data-src` vs `src`) fix; src FIRST the
  ...
  it('keeps a TINY inlined image (< 133 base64 chars) that Readability would treat as a placeholder', () => {
    // Readability's size-based branch drops a short data: src when another
    // attribute holds an image URL; with the data-* copies stripped it stays.
    const tiny =
      'data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk';
    const html = applyInlinedImages(
      article(`<img class="lazyloaded" data-src="${REMOTE}" src="${REMOTE}" alt="icon">`),
      [{ src: REMOTE, dataUri: tiny }],
    );

    const extract = parseReader(html, REMOTE);

    expect(extract.content).toContain(tiny);
  });

  it('leaves an un-captured LQIP placeholder to Readability, which restores the remote URL (unchanged behavior)', () => {
    // The capture skips an <img> whose src is already data: (a page-authored
    // blur-up placeholder), so the tag is not rewritten and Readability swaps in
    // the real URL from data-src exactly as before — the PDF path renders it.
    const lqip = `data:image/jpeg;base64,${'/9j/4AAQSkZJRgABAQAAAQABAAD'.repeat(10)}`;
    const extract = parseReader(
      article(`<img class="lazyload" src="${lqip}" data-src="${REMOTE}" alt="photo">`),
      REMOTE,
    );

    expect(extract.content).toContain(`src="${REMOTE}"`);
    expect(extract.content).not.toContain(lqip);
  });
